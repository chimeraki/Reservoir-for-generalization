...
for cho in range(nono):
     for i in range(boo):
         ...
         for t in range(dt+start):
             ...
             x1=lam*tanh(intot + dot(adj_res,x1))
...
for i in range(l):
        ...
        for t in range(dt+start):
                ...
                xout1=lam*tanh(outtot + dot(adj_res,xout1))
                if t==start:
                        Xout1=(xout1)
                else:
...
plt.savefig('single_scalenblurnnoise'+"{0:.0f}".format(correct)+'.jpg')
plt.close()
