...
training_single={}
train_sizz=linspace(0,500,21)
train_sizz=[0,5,15,25,35,45,55,65,75,85,95]
for r in range(len(train_sizz)-1):
     ...
     for soo in range(1):
          ...
          for jiggs in range(labels):
               ...
               print('success rate for training size ',boo*5,'/t',jiggs, correct/(i+1))
          total= total/((jiggs+1)*(i+1))
          training_single[boo*5]=total




plt.scatter(list( training_single.keys()),list( training_single.values()))
xlabel('$Training \quad Data \quad Size$', fontsize=13)
ylabel('$Fraction \quad correct$', fontsize=13)
plt.savefig('Single_trainingsize',pad_inches=1)
plt.close()
